...
def main():
    ...
    classifier = ClassifierHead(d_model=D_MODEL, num_classes=VOCAB_SIZE)

    decoder_output = decoder(input_ids)
    class_logits = classifier(decoder_output)

    print(class_logits)
    print(class_logits.shape)

    next_token_batch1 = class_logits[0, -1].argmax().item()
    next_token_batch2 = class_logits[1, -1].argmax().item()

    print(f"Most likely next token batch 1: {next_token_batch1}")
    print(f"Most likely next token batch 2: {next_token_batch2}")
