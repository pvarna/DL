...
from dl_lib.nn import Conv3d


class TestConv3dInit(unittest.TestCase):
    ...
    def test_when_called_with_valid_arguments_then_initializes_attributes(
        ...
        # Act
        layer = Conv3d(in_channels, out_channels, kernel_size, stride, padding)
    ...
    def test_when_stride_not_specified_then_defaults_to_one(self):
        ...
        # Act
        layer = Conv3d(in_channels=1, out_channels=1, kernel_size=3)
        actual = layer.stride
    ...
    def test_when_constructed_with_tuple_kernel_then_weight_shape_is_correct(
        ...
        out_channels = 16
        kernel_size = (3, 5, 7)
        expected = (out_channels, in_channels, 3, 5, 7)
    ...
    def test_when_constructed_then_weights_are_in_valid_range(self):
        ...
        in_channels = 3
        kernel_size = 3
        sqrt_k = (1 / (in_channels * kernel_size**3))**0.5

        # Act
        layer = Conv3d(in_channels, out_channels=16, kernel_size=kernel_size)
...
class TestConv3dForward(unittest.TestCase):
    ...
    def test_when_called_then_returns_tensor(self):
        ...
        # Act
        actual = Conv3d(in_channels=1, out_channels=1, kernel_size=3)(x)
    ...
    def test_when_called_then_output_shape_is_correct(self):
        ...
        # Act
        actual = Conv3d(in_channels=1, out_channels=1, kernel_size=3)(x).shape
    ...
    def test_when_called_with_stride_2_then_output_shape_is_correct(self):
        ...
        # Act
        actual = Conv3d(in_channels=1, out_channels=1, kernel_size=3,
                        stride=2)(x).shape
    ...
    def test_when_called_with_multiple_input_channels_then_output_shape_is_correct(
        ...
        # Act
        actual = Conv3d(in_channels=3, out_channels=16, kernel_size=3)(x).shape
    ...
    def test_when_called_with_known_weights_then_output_matches_expected(self):
        # Arrange
        layer = Conv3d(in_channels=1,
                       out_channels=1,
                       kernel_size=3,
                       bias=False)
        layer.weight = torch.ones(1, 1, 3, 3, 3)
        x = torch.ones(1, 1, 3, 3, 3)
        expected = 27.0

        # Act
        actual = layer(x)[0, 0, 0, 0, 0].item()
    ...
    def test_when_bias_is_false_then_output_does_not_include_bias(self):
        ...
        layer = Conv3d(in_channels=1,
                       ...
                       kernel_size=3,
                       bias=False)
        layer.weight = torch.zeros(1, 1, 3, 3, 3)
        x = torch.randn(1, 1, 5, 5, 5)
        expected = torch.zeros(1, 1, 3, 3, 3)
