...
BATCH_SIZE = 32
IMG_SIZE = 224
TRAIN_DIR = os.path.join("drive", "MyDrive", "DL_HW1", "Homework01", "DATA",
                         "train")
VAL_DIR = os.path.join("drive", "MyDrive", "DL_HW1", "Homework01", "DATA",
                       "val")
TEST_DIR = os.path.join("drive", "MyDrive", "DL_HW1", "Homework01", "DATA",
                        "test")
NUM_CLASSES = 15
LEARNING_RATE = 0.001
NUM_EPOCHS = 50
...
PRETRAINED_MODEL_SAVE_PATH = "best_resnet50_finetuned.pt"
...
def main():
    ...
    optimizer_ft = optim.AdamW(resnet_model.parameters(),
                               lr=LEARNING_RATE,
                               weight_decay=0.01)
    ...
    loss_ft, acc_ft, f1_ft, per_class_dict_ft = evaluate_model(
        test_load, resnet_model, criterion)

    pretty_dict_ft = {
        cls: f"{score:.3f}"
        for cls, score in per_class_dict_ft.items()
    }
    print(f"ResNet-50 Test Avg Loss: {loss_ft:.3f}")
    print(f"ResNet-50 Test Accuracy:  {acc_ft:.3f}")
    print(f"ResNet-50 Test Macro F1:  {f1_ft:.3f}")
    print(f"ResNet-50 Test Per-class F1: {pretty_dict_ft}")
