...
def calculate_loss(w1, w2, bias, dataset):
    losses = [(y - (x1 * w1 + x2 * w2 + bias))**2 for x1, x2, y in dataset]
...
def main():
    ...
    # w1 and w2 for both models are now close to 0.5
    # in the previous task they were close to 0.33 for AND and 0.66 for OR
