...
class MLP:
    ...
    def __init__(self, in_channels: int, hidden_channels: list[int]) -> None:
        ...
        self.layers = [
            Linear(sizes[i], sizes[i + 1]) for i in range(len(hidden_channels))
        ]
...
def main() -> None:
    ...
    for _ in range(10):
        print(
            f'Gradient of the first weight in the first neuron in the first layer: {n.layers[0].neurons[0].w[0].gradient}.'
        )
        ...
        for parameter in n.parameters():
            parameter.data -= 0.1 * parameter.gradient

    y_preds = [n(x) for x in xs]
    print('Predictions:')
    print([y_pred.data for y_pred in y_preds])

    # The gradient is exploding
    
    # The gradients are not reset after each epoch, so they accumulate across epochs
    # The gradients should be reset to zero after each backward pass
