...
def word_probability(word, bigram_matrix, vocabulary):
    char_to_idx = {ch: i for i, ch in enumerate(vocabulary)}
    full_sequence = ["."] + list(word) + ["."]

    prob = 1.0
    for prev, next in zip(full_sequence, full_sequence[1:]):
        i = char_to_idx[prev]
        j = char_to_idx[next]
        p = bigram_matrix[i, j].item()
        prob *= p
...
def main():
    ...
    word = "simqeon"
    prob = word_probability(word, bigram_matrix, vocabulary)
    print(prob)

    char_to_idx = {ch: i for i, ch in enumerate(vocabulary)}
    print(
        f"Probability of 'mq': {bigram_matrix[char_to_idx['m'], char_to_idx['q']]}"
    )
    print(
        f"Probability of 'qe': {bigram_matrix[char_to_idx['q'], char_to_idx['e']]}"
    )

    # The probability of the word "simqeon" is 0.0 becuase the model assigns 0 probability to the bigram "mq"
    # It never appeared in the training data
    # Smoothing is needed
